...
public class ColumnIdExpr {
    ...
    private static void setColumnId(Map<String, Column> nameToColumn, Expr expr) {
        if (expr instanceof SlotRef) {
            ...
            slotRef.setColumnId(column.getColumnId().toString());
        }
    ...
    private static class ExprSerializeVisitor extends AST2StringVisitor {
        ...
        public String visitSlot(SlotRef node, Void context) {
            if (node.getTblNameWithoutAnalyzed() != null) {
                return node.getTblNameWithoutAnalyzed().toString() + "." + ParseUtil.backquote(node.getColumnId());
            } else {
                String colId = node.getColumnId();
                return node.isBackQuoted() ? ParseUtil.backquote(colId) : colId;
            }
        }
    }
}
