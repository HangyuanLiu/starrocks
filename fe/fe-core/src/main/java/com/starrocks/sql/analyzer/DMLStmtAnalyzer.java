...
public class DMLStmtAnalyzer {
    ...
    static class DMLStmtAnalyzerVisitor implements AstVisitor<Void, ConnectContext> {
        public void analyze(DmlStmt dmlStmt, ConnectContext context) {
            ...
            if (context.getGlobalTransactionLoadState() != null) {
                GlobalTransactionState globalTransactionState = context.getGlobalTransactionLoadState();
                List<GlobalTransactionState.TransactionStateItem> transactionStateItemList =
                        globalTransactionState.getTransactionStateItems();

                for (GlobalTransactionState.TransactionStateItem transactionStateItem : transactionStateItemList) {
