...
public class SemiReorderRule extends TransformationRule {
    public SemiReorderRule() {
        super(RuleType.TF_SEMI_REORDER, Pattern.create(OperatorType.LOGICAL_JOIN).
                addChildren(
    ...
    public boolean check(final OptExpression input, OptimizerContext context) {
        ...
        if (!topJoin.getJoinType().isLeftSemiAntiJoin()) {
            return false;
        }

        ColumnRefSet c = new ColumnRefSet();
        c.union(input.inputAt(0).inputAt(0).getOutputColumns());
        c.union(input.inputAt(1).getOutputColumns());
        if (!c.contains(topJoin.getOnPredicate().getUsedColumns())) {
            return false;
        }
