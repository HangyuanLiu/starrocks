...
public class QueryAnalyzerV2 {
    ...
    //private final AnalyzeState analyzeState = new AnalyzeState();
    ...
    private class Visitor extends AstVisitor<Scope, Scope> {
        ...
        @Override
        public Scope visitQuery(QueryStatement node, Scope parent) {
        ...
        @Override
        public Scope visitSelect(SelectRelation stmt, Scope scope) {
            AnalyzeState analyzeState = new AnalyzeState();

            Relation resolvedRelation = resolveTableRef(stmt.getRelation(), scope);
            stmt.setRelation(resolvedRelation);
            ...
            if (stmt.hasLimit()) {
                if (stmt.getOffset() > 0 && orderByElements.isEmpty()) {
                    ...
                    analyzeState.setLimit(new LimitElement(0, stmt.getLimitNum()));
                } else {
                    analyzeState.setLimit(new LimitElement(stmt.getOffset(), stmt.getLimitNum()));
                }
            }

            stmt.fillAnalyzeState(analyzeState);
            return outputScope;
        }
        ...
        private List<Expr> analyzeSelect(SelectRelation stmt, AnalyzeState analyzeState, Scope scope) {
        ...
        private Relation resolveTableRef(Relation relation, Scope scope) {
            if (relation instanceof JoinRelation) {
                ...
                //TableName tableName = tableRelation.getAlias();
                ...
                if (table instanceof View) {
                    ...
                    QueryStatement stmt = (QueryStatement) new AstBuilder().visitSingleStatement(singleStatementContext);

                    return new SubqueryRelation(tableName.getTbl(), stmt.getQueryRelation());
                } else {
        ...
        public Scope visitSubquery(SubqueryRelation subquery, Scope context) {
            Scope queryOutputScope = process(subquery.getQuery(), context);
        ...
        public Scope visitTableFunction(TableFunctionRelation node, Scope scope) {
            ...
            for (int i = 0; i < args.size(); ++i) {
                ...
                verifyNoAggregateFunctions(args.get(i), "UNNEST");
                verifyNoWindowFunctions(args.get(i), "UNNEST");
                verifyNoGroupingFunctions(args.get(i), "UNNEST");
            }
    ...
    public void analyzeWhere(SelectRelation stmt, AnalyzeState analyzeState, Scope scope) {
    ...
    private List<Expr> analyzeGroupBy(SelectRelation node, AnalyzeState analyzeState, Scope sourceScope,
    ...
    private void analyzeHaving(SelectRelation node, AnalyzeState analyzeState,
    ...
    private Scope computeAndAssignOutputScope(SelectRelation stmt, Scope scope) {
        ImmutableList.Builder<Field> outputFields = ImmutableList.builder();
        ...
        stmt.setScope(outputScope);
        return outputScope;
    }

    private List<OrderByElement> analyzeOrderBy(SelectRelation node, AnalyzeState analyzeState, Scope orderByScope,
                                                List<Expr> outputExpressions) {
    ...
    private void verifyNoAggregateFunctions(Expr expression, String clause) {
        ...
        expression.collectAll((com.google.common.base.Predicate<Expr>) arg -> arg instanceof FunctionCallExpr &&
    ...
    private void verifyNoWindowFunctions(Expr expression, String clause) {
        ...
        expression.collectAll((com.google.common.base.Predicate<Expr>) arg -> arg instanceof AnalyticExpr, functions);
        if (!functions.isEmpty()) {
