        ...
        try {
            ...
            if (!nodeMgr.getMySelf().getNodeName().equals(frontend.getNodeName())) {
                String responseContent =
                        StarRocksHttpClient.redirect(frontend.getHost(), frontend.getRpcPort(), request.getRequest());
            ...
            if (context == null) {
                throw new AuthenticationException("Not found connect context " + connectionId);
            }
            ...
            response.appendContent(OAuth2ResultMessage.generateLoginSuccessPage(
                    "Fail", e.getMessage(), "", String.valueOf(connectionId)));
            sendResult(request, response);
        }
    }
