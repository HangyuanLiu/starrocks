    ...
    public void dropDb(String dbName, boolean isForceDrop) throws DdlException, MetaNotFoundException {
        ...
        try {
            ...
            for (Policy policy : allMaskingPolicy.values()) {
                if (!isForceDrop && securityPolicyManager.isPolicyHasApplied(PolicyType.COLUMN_MASKING, policy.getPolicyId())) {
                    throw new DdlException("");
                }
            }

            Map<String, Policy> allRowAccessPolicy = securityPolicyManager
                    .getNameToPolicy(InternalCatalog.DEFAULT_INTERNAL_CATALOG_NAME, dbName, PolicyType.ROW_ACCESS);
            for (Policy policy : allRowAccessPolicy.values()) {
                if (!isForceDrop && !securityPolicyManager.isPolicyHasApplied(PolicyType.ROW_ACCESS, policy.getPolicyId())) {
                    throw new DdlException("");
                }
            }
            ...
            for (Policy policy : allMaskingPolicy.values()) {
                DropPolicyInfo dropPolicyInfo = new DropPolicyInfo(policy.getPolicyType(),
                        policy.getPolicyId(), policy.getDbPEntryObject(), policy.getName());
                dropPolicyInfos.add(dropPolicyInfo);
