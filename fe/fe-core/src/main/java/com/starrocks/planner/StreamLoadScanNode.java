 ...
 * conversion pipeline for Stream Load. These expressions are serialized directly to Thrift via
        ...
        for (SlotDescriptor dstSlotDesc : desc.getSlots()) {
            ...
            if (expr == null) {
                ...
                if (srcSlotDesc != null) {
                    ...
                    SlotRef slotRef = new SlotRef(null, dstSlotDesc.getColumn().getName());
                    slotRef.setSlotId(srcSlotDesc.getId().asInt());
                    if (srcSlotDesc.getParent() != null) {
                        slotRef.setTupleId(srcSlotDesc.getParent().getId().asInt());
                    }
                    slotRef.setType(srcSlotDesc.getType());
                    slotRef.setNullable(srcSlotDesc.getIsNullable());
                    expr = slotRef;
                } else {
            ...
            paramCreateContext.params.putToExpr_of_dest_slot(dstSlotDesc.getId().asInt(),
                    ExecExprSerializer.serialize(execExpr));
        }
