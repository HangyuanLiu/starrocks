...
StarRocksDataSourceProvider::StarRocksDataSourceProvider(ConnectorScanNode* scan_node, const TPlanNode& plan_node)
        : _scan_node(scan_node), _scan_node_thrift(plan_node.starrocks_scan_node) {
    _execution_context.connector_name = "starrocks";
    ...
    for (size_t i = 0; i < slots.size(); ++i) {
        ...
        if (slot->is_nullable()) {
            ...
            fill_null_column(arrow_array.get(), 0, batch.num_rows(),
                           nullable_column->null_column_raw_ptr(), 0);
            null_data = nullable_column->null_column_raw_ptr()->get_data().data();
            data_column = nullable_column->data_column_raw_ptr();
        }
        
        RETURN_IF_ERROR(conv_func(arrow_array.get(), 0, batch.num_rows(), data_column, 0,
