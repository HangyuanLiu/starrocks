...
// macOS override of `be/src/fs/fs.cpp`.
// WHY: StarRocks' upstream implementation disables S3/HDFS backends on __APPLE__.
// format-lib must be able to access Lake data on object storage (S3 compatible),
// so we enable S3 on macOS while keeping HDFS/Azure disabled.
...
StatusOr<std::unique_ptr<FileSystem>> FileSystem::CreateUniqueFromString(std::string_view uri,
    ...
    return new_fs_hdfs(options);
#else
    return Status::NotSupported(fmt::format("Unsupported filesystem uri on macOS: {}", uri));
#endif
}
